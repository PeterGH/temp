...
{
    Add("Two Sum UnSorted MultiSolutions1", [&]() {
        {
            auto check = [&](vector<int> & input, int sum) {
                Logger() << input;
                Logger() << "Target: " << sum << endl;

                vector<pair<int, int>> pairs1 = LeetCode::TwoSum::unsortedMultipleSolutions(input, sum);
                vector<pair<int, int>> pairs2 = LeetCode::TwoSum::unsortedMultipleSolutions2(input, sum);

                sort(pairs1.begin(), pairs1.end());
                sort(pairs2.begin(), pairs2.end());
                Logger().WriteInformation("pairs count %d %s %d\n", pairs1.size(), pairs1.size() == pairs2.size() ? "==" : "!=", pairs2.size());
                Logger() << pairs1;
                Logger() << pairs2;
                ASSERT1(pairs1.size() == pairs2.size());
                for (size_t i = 0; i < pairs1.size(); i++) {
                    Logger().WriteInformation("pairs %d: <%d,%d> %s <%d,%d>\n", i, pairs1[i].first, pairs1[i].second, pairs1[i] == pairs2[i] ? "==" : "!=", pairs2[i].first, pairs2[i].second);
                    ASSERT1(pairs1[i] == pairs2[i]);
                }
            };
            {
                vector<int> input = { -3, 0, -6, 2, 1, 19, -18, 5, 8, -14, -1, 14, -3, 19, 20, -18, -9, 10,  4, -19, -12, -13, 10, -15, -20, 17,  0, 18, -10, 15, 17, -9, -3,  4,  7, -19, -2,  0, -7, -17, 11, 19, 14 };
    ...
    Add("Two Sum UnSorted MultiSolutions2", [&]() {
        {
            auto check = [&](vector<int> & input, int sum) {
                vector<pair<int, int>> pairs1 = LeetCode::TwoSum::unsortedMultipleSolutions(input, sum);
                vector<pair<int, int>> pairs2 = LeetCode::TwoSum::unsortedMultipleSolutions2(input, sum);
                ...
                sort(pairs1.begin(), pairs1.end());
                sort(pairs2.begin(), pairs2.end());
                Logger().WriteInformation("Target: %d, pairs count %d %s %d\n", sum, pairs1.size(), pairs1.size() == pairs2.size() ? "==" : "!=", pairs2.size());
                ASSERT1(pairs1.size() == pairs2.size());
                for (size_t i = 0; i < pairs1.size(); i++) {
            ...
            {
                for (int i = 0; i < 20; i++) {
                    vector<int> input = Util::RandomVector(Util::RandomInt(50, 2), 20, -20);
                    int length = (int)input.size();
    ...
    Add("Two Sum Sorted MultiSolutions", [&]() {
        {
            auto check = [&](vector<int> & input, int sum) {
                vector<pair<int, int>> pairs1 = LeetCode::TwoSum::sortedMultipleSolutions(input, sum);
                vector<pair<int, int>> pairs2 = LeetCode::TwoSum::sortedMultipleSolutions2(input, sum);
    ...
    Add("31. Next Permutation", [&]() {
        auto check = [&](vector<int>& v)
        {
            ...
            Logger() << v;
            vector<int> v2 = Util::Duplicate(v);
            vector<int> v3 = Util::Duplicate(v);
            while (std::next_permutation(v.begin(), v.end()))
            {
                LeetCode::Permutation::nextPermutation(v2);
                LeetCode::Permutation::nextPermutation(v3);
                Logger() << v2;
                for (size_t i = 0; i < v.size(); i++)
                {
                    ASSERT1(v[i] == v2[i]);
        ...
        for (int i = 0; i < 10; i++) {
            vector<int> input = Util::RandomVector(Util::RandomInt(6, 1), 6);
            int length = (int)input.size();
            Logger().WriteInformation("Run %d: %d elements\n", i, length);
            check(input);
        }
        for (int i = 0; i < 100; i++) {
            vector<int> v = Util::RandomVector(Util::RandomInt(10, 1), 10);
    ...
    Add("46. Permutation", [&]() {
        auto check = [&](vector<int>& v)
        {
            ...
            while (std::next_permutation(v.begin(), v.end())) p.push_back(vector<int>(v));
            Logger() << p;
            Util::Sort(p);
            vector<int> v2 = Util::Duplicate(v);
            vector<vector<int>> p2 = LeetCode::Permutation::permute(v2);
            Logger() << p2;
            Util::Sort(p2);
            vector<int> v3 = Util::Duplicate(v);
            vector<vector<int>> p3 = LeetCode::Permutation::permute2(v3);
            Logger() << p3;
            Util::Sort(p3);
            vector<int> v4 = Util::Duplicate(v);
    ...
    Add("40. Combination Sum No Reuse", [&]() {
        auto check = [&](vector<int>& v, int t) {
            Logger() << v << "Target " << t << endl;
            auto v1 = Util::Duplicate(v);
            auto v2 = Util::Duplicate(v);
