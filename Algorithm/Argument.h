...
namespace Test {
	...
	private:
		vector<basic_string<T>> indexed; // Indexed arguments
		map<basic_string<T>, basic_string<T>> named; // Named arguments
	...
	template<class T> bool Argument<T>::Has(const basic_string<T> & key) const
	{
		basic_string<T> keyl = String::ToLower(key);
		return this->named.find(keyl) != this->named.end();
	}

	template<class T> const basic_string<T> & Argument<T>::operator[](const basic_string<T> & key) const
	{
		basic_string<T> keyl = String::ToLower(key);
	...
	template<class T> const basic_string<T> & Argument<T>::operator[](unsigned int index) const
	{
	...
	template<class T> const basic_string<T> Argument<T>::ToString(void) const
	{
		basic_string<T> s;
		unsigned int i = 0;
		string t;
		for (map<basic_string<T>, basic_string<T>>::const_iterator it = this->named.cbegin(); it != this->named.cend(); it++) {
			t = "-";
	...
	template<class T> const set<basic_string<T>> Argument<T>::Keys(void) const
	{
