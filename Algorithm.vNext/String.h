...
namespace Test {
	...
	public:
		...
		template<class O, class I> static basic_string<O> ToString(const basic_string<I> & input)
		{
		...
		template<class T> static basic_string<T> ToLower(const basic_string<T> & input)
		{
		...
		template<class T> static bool StartsWith(const basic_string<T> & input1, const basic_string<T> & input2)
		{
			return 0 == ToLower(input1).compare(0, input2.size(), ToLower(input2));
		}

		// Check if string input1 ends with string input2
		template<class T> static bool EndsWith(const basic_string<T> & input1, const basic_string<T> & input2)
		{
		...
		template<class T> static bool Contains(const basic_string<T> & input1, const basic_string<T> & input2)
		{
		...
		template<class T> static basic_string<T> TrimStart(const basic_string<T> & input, const T * characters)
		{
		...
		template<class T> static basic_string<T> TrimEnd(const basic_string<T> & input, const T * characters)
		{
			if (input.length() == 0) return input;
		...
		template<class T> static basic_string<T> Trim(const basic_string<T> & input, const T * characters)
		{
			if (input.length() == 0) return input;
		...
		template<class T, class C> static basic_string<C> Join(const T * input, size_t length, const C * separator)
		{
		...
		template<class T> static basic_string<T> Random(const basic_string<T> & alphabet, size_t length)
		{
			basic_string<T> output;
		...
		template<class T> static basic_string<T> Random(size_t length, const basic_string<T> & alphabet = "abcdefghijklmnopqrstuvwxyz")
		{
